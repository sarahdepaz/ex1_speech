...
def dtw(mfcc1, mfcc2):
    dtw_mat = np.zeros([101, 101], np.float64)
...
def euclid_mat_helper(mfcc1, mfcc2):
    euclid_mat = np.zeros([101, 101], np.float32)
...
def knn(label, files):
    ...
    for filename in os.listdir(directory):
        ...
        for i in range(len(files)):
            ...
            mfcc1 = librosa.feature.mfcc(y=y1, sr=sr1, n_mfcc=13, n_fft=1024, hop_length=int(sr1 * 0.01))
            mfcc_d1 = librosa.feature.delta(mfcc1)
            mfcc1 = np.concatenate([mfcc1, mfcc_d1])
            mfcc1 = normalize(mfcc1)
...
def euclidian_dist(label):
    ...
    for filename in os.listdir(directory):
        ...
        for j in range(len(files)):
            ...
            mfcc_train = librosa.feature.mfcc(y=y1, sr=sr1, n_mfcc=13, n_fft=1024, hop_length=int(sr1 * 0.01))
            mfcc_d1 = librosa.feature.delta(mfcc_train)
            mfcc_train = np.concatenate([mfcc_train, mfcc_d1])
